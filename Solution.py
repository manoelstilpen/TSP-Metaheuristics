...
class GreedCheapestSolution(Solution):
    """ Construct a Solution object by Cheapest Insertion procedure. """
    def __init__(self, distances):
...
class GreedNearestSolution(Solution):
    ...
    def construct(self):
        # Create a empty route allocating all positions as source city - 0 (first and last position must be source city, forming a cycle).
        route = [0 for _ in range(self.n_cities+1)]
...
class RandomSolution(Solution):
    ...
    def construct(self):
        import random
        aux = list(range(1, len(self.distances)))
        random.shuffle(aux)
        return [0] + aux + [0]
